...
import { Upload, Download, CheckCircle2, Loader2, PenTool, FileText, X, Info, Sparkles } from 'lucide-react'

interface PDFSignTemplateProps {
  title: string
  description: string
  endpoint: string
}

export default function PDFSignTemplate({ title, description, endpoint }: PDFSignTemplateProps) {
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-gray-900 flex items-center justify-center">
                <PenTool size={24} className="text-white" strokeWidth={2} />
              </div>
              <div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-300 shadow-sm overflow-hidden flex-1 flex items-center justify-center">
                <div className="text-center py-12">
                  <div className="inline-block mb-4">
                    <PenTool size={48} className="text-gray-300" strokeWidth={1.5} />
                  </div>
                  <p className="text-sm text-gray-500">Upload a PDF file to sign</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col">
            {!result ? (
              <div className="bg-white rounded-2xl border border-gray-300 shadow-sm overflow-hidden flex-1 flex flex-col">
                <div className="border-b border-gray-200 px-6 py-4 bg-white">
                  <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Sign PDF</h2>
                </div>
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center text-gray-500">
                      {file ? 'Click the button below to sign your PDF' : 'Upload a PDF to get started'}
                    </div>
                  </div>
                  {file && (
                    <motion.button
              ...
              >
                <div className="border-b border-emerald-200 px-6 py-4 bg-emerald-50">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-sm font-semibold text-emerald-900 uppercase tracking-wide">PDF Signed</h2>
                  </div>
                </div>
                <div className="p-6">
