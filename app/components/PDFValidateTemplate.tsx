...
import { Upload, CheckCircle2, Loader2, AlertCircle, FileText, X, Info, Sparkles, ShieldCheck } from 'lucide-react'

interface PDFValidateTemplateProps {
  title: string
  description: string
  endpoint: string
}

export default function PDFValidateTemplate({ title, description, endpoint }: PDFValidateTemplateProps) {
  const [file, setFile] = useState<File | null>(null)
  const [isProcessing, setIsProcessing] = useState(false)
  const [result, setResult] = useState<{ valid: boolean; pages?: number } | null>(null)
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-blue-900">
                      ...
                      <ul className="space-y-1 text-blue-800">
                        ...
                        <li>• System checks PDF validity</li>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-300 shadow-sm overflow-hidden flex-1 flex items-center justify-center">
                <div className="text-center py-12">
                  <div className="inline-block mb-4">
                    <ShieldCheck size={48} className="text-gray-300" strokeWidth={1.5} />
                  </div>
                  <p className="text-sm text-gray-500">Upload a PDF file to validate</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col">
            {!result ? (
              <div className="bg-white rounded-2xl border border-gray-300 shadow-sm overflow-hidden flex-1 flex flex-col">
                <div className="border-b border-gray-200 px-6 py-4 bg-white">
                  <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wide">Validation Result</h2>
                </div>
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center text-gray-500">
                      {file ? 'Click the button below to validate your PDF' : 'Upload a PDF to get started'}
                    </div>
                  </div>
                  {file && (
                    <motion.button
              ...
              <motion.div
                ...
                className={`bg-white rounded-2xl border shadow-lg overflow-hidden ${
                  result.valid ? 'border-emerald-300' : 'border-red-300'
                }`}
              >
                  ...
                  <div className="flex items-center gap-2">
                    {result.valid ? (
                      <>
                        ...
                        <h2 className="text-sm font-semibold text-emerald-900 uppercase tracking-wide">Valid PDF</h2>
                      </>
                    ) : (
                      <>
                        <AlertCircle size={20} className="text-red-600" />
                        <h2 className="text-sm font-semibold text-red-900 uppercase tracking-wide">Invalid PDF</h2>
                      </>
                    )}
                  </div>
                </div>
                <div className="p-6">
                  <div className="space-y-4">
                    {result.valid && result.pages && (
                      <div className="bg-gray-50 rounded-lg p-4">
                        <p className="text-sm text-gray-600">Total Pages</p>
                        <p className="text-2xl font-bold text-gray-900">{result.pages}</p>
                      </div>
                    )}
                    <motion.button
                      whileHover={{ scale: 1.02 }}
